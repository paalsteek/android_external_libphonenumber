...
/**
 * Flyweight area code map storage strategy that uses a table to store unique
 * strings and shorts to store the prefix and description indexes when possible.
 * It is particularly space-efficient when the provided area code map contains a
 * lot of redundant descriptions.
 * 
 * @author Philippe Liard
 */
final class FlyweightMapStorageExt extends AreaCodeMapStorageStrategy {
    ...
    // The number of bytes used to store a description index. It is computed
    // from the size of the
    // description pool containing all the strings.
    private int descIndexSizeInBytes;
    private ByteBuffer numberDescriptionIndexes;

    // Sorted string array of unique description strings.
    private String[] descriptionPool;

    private static void storeWordInBufferOffset(ByteBuffer buffer, int wordSize,
    ...
    private int readWordFromBufferOffset(ByteBuffer buffer, int wordSize, int offset) {
        return wordSize == SHORT_NUM_BYTES ? buffer.getShort(offset)
                : buffer.getInt(offset);
    }

    @Override
    public int getPrefix(int index) {
        int offset = index * (prefixSizeInBytes + descIndexSizeInBytes);
        return readWordFromBufferOffset(numberDescriptionIndexes, prefixSizeInBytes,
                offset);
    }

    /**
     * This implementation returns the same string (same identity) when called
     * for multiple indexes corresponding to prefixes that have the same
     * description.
     */
    @Override
    public String getDescription(int index) {
        int offset = index * (prefixSizeInBytes + descIndexSizeInBytes) + prefixSizeInBytes;

        int indexInDescriptionPool = readWordFromBufferOffset(numberDescriptionIndexes,
    ...
    public void readFromSortedMap(SortedMap<Integer, String> areaCodeMap) {
        ...
        numberDescriptionIndexes = ByteBuffer.allocate(numOfEntries
                * (prefixSizeInBytes + descIndexSizeInBytes));

        // Fill the phone number prefixes byte buffer, the set of possible
        // lengths of prefixes and the
        // description set.
        int index = 0;
        for (Entry<Integer, String> entry : areaCodeMap.entrySet()) {
            int prefix = entry.getKey();
            int offset = index * (prefixSizeInBytes + descIndexSizeInBytes);
            storeWordInBufferOffset(numberDescriptionIndexes, prefixSizeInBytes, offset, prefix);
    ...
    private void createDescriptionPool(SortedSet<String> descriptionsSet,
        ...
        for (int i = 0; i < numOfEntries; i++) {
            int offset = i * (prefixSizeInBytes + descIndexSizeInBytes);
            int prefix = readWordFromBufferOffset(numberDescriptionIndexes, prefixSizeInBytes,
                    offset);
            String description = areaCodeMap.get(prefix);
            int positionInDescriptionPool = Arrays.binarySearch(descriptionPool, description);
            storeWordInBufferOffset(numberDescriptionIndexes, descIndexSizeInBytes, offset
                    + prefixSizeInBytes,
    ...
    /**
     * Reads the area code entries from the provided input stream and stores
     * them to the internal byte buffers.
     */
    private void readEntries(ObjectInput objectInput) throws IOException {
        numOfEntries = objectInput.readInt();

        final int total = numOfEntries * (prefixSizeInBytes + descIndexSizeInBytes);
        final int readBufferSize = 1024;
        byte[] buffer = new byte[total];
        int iRead = 0;
        int offset = 0;
        do {
            iRead = objectInput.read(buffer, offset,
                    (offset + readBufferSize) < total ? readBufferSize : total - offset);
            offset += iRead;
        } while (iRead != -1 && offset < total);
        numberDescriptionIndexes = ByteBuffer.wrap(buffer);
    ...
    public void writeExternal(ObjectOutput objectOutput) throws IOException {
        ...
        objectOutput.write(numberDescriptionIndexes.array());
    }

    /**
     * Gets the minimum number of bytes that can be used to store the provided
     * {@code value}.
     */
    ...
    /**
     * Reads the {@code value} at the specified {@code index} from the provided
     * byte {@code buffer}. Note that only integer and short sizes are
     * supported.
     * 
    ...
    /**
     * Stores the provided {@code value} to the provided byte {@code buffer} at
     * the specified {@code index} using the provided {@code wordSize} in bytes.
     ...
     * @param value the value that is stored assuming it does not require more
